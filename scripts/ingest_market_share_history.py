...
# صادرات جهانی روسیه (HS ۲۸۳۶۳۰) — کاربر داده‌ی چندساله‌ی تناژی (۲۰۲۱ تا
# ۲۰۲۶) داد، ولی فقط ۲۰۲۱ (۳۴٫۹۰ میلیون دلار طبق خودِ کاربر) رقم دلاریِ دقیق
# داشت؛ ۲۰۲۲ تا ۲۰۲۵ فقط بازه‌ی تناژِ برآوردی بودن (بدون رقم دلاری) و ۲۰۲۶
# هدف‌گذاریه، نه واقعی. اما معلوم شد خودِ data/trade_map_2025.json (خروجی
# scripts/ingest_trade_map.py، ITC Trade Map — همون منبعی که کارت‌های سربرگ
# صفحه‌ی رقیب روسیه رو تغذیه می‌کنه) از قبل یک روند ۵ساله‌ی کامل و دقیق داره
# (export_trend.values_usd_k برای ۲۰۲۱-۲۰۲۵)، که عددش برای ۲۰۲۱ (۳۴,۹۰۴ هزار
# دلار) دقیقاً با رقم گمرکیِ کاربر یکی درمیاد — یعنی اعتبارسنجی متقابل شده.
# پس به‌جای رقم دستی فقط برای یک سال، این تابع مستقیم از همون فایل می‌خونه و
# هر ۴ سالی که توی GLOBAL_YEARS هست (۲۰۲۱-۲۰۲۴) رو با رقم دقیق پر می‌کنه —
# نه فقط یکی. تناژِ برآوردیِ کاربر برای هر سال (که با این ارقام دلاری همیشه
# هم‌خوان نیست، چون منبع/دامنه‌ی محصول فرق داره) در پروفایل رقیب روسیه
# (data/competitors.json → export_volume_trend) کنار هم نگه داشته شده، نه اینجا.
def _load_russia_trade_map_exports():
    path = os.path.join(DATA_DIR, "trade_map_2025.json")
    if not os.path.exists(path):
        return {}
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    trend = (data.get("روسیه") or {}).get("export_trend")
    if not trend:
        return {}
    return {
        year: {"value_usd_k": value}
        for year, value in zip(trend["years"], trend["values_usd_k"])
        if year in GLOBAL_YEARS and value is not None
    }


RUSSIA_CUSTOMS_EXPORTS = _load_russia_trade_map_exports()
...
def build_global_market():
    ...
    return {
        ...
        "note_fa": (
            ...
            "آمار WITS عدد کامل دارند، به‌علاوه‌ی ایران و روسیه برای سال‌هایی که "
            "منبع دستیِ دلاریِ دقیق و مستند دارن — نه کل صادرات جهانی. برای "
        ...
        "manual_overrides": {
            fa: {
                "years": ys,
                "source": {
                    "ایران": "آمار رسمی گمرک جمهوری اسلامی ایران (کد تعرفه ۲۸۳۶۳۰۹۰، بدون گرید دارویی)",
                    "روسیه": "ITC Trade Map (data/trade_map_2025.json) — هم‌راستا با رقم رسمی گمرک FTS/UN Comtrade که کاربر برای ۲۰۲۱ ارائه داد",
                }.get(fa, "داده‌ی ارائه‌شده توسط کاربر"),
            }
