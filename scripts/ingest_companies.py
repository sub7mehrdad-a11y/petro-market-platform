...
SOURCES = [
    ...
    {"file": "Uzbekistan_Companies.xlsx", "country": "ازبکستان"},
    # فایل اصلی کاربر (۲۰۲۶-۰۹-۱۶) ۳ سطر عنوان/زیرعنوان/خالی قبل از سرستون واقعی
    # داشت؛ چون load_companies همیشه rows[0] رو سرستون فرض می‌کنه (بدون گزینه‌ی
    # header_row مثل TABLE_SOURCES توی ingest_exhibitions.py)، نسخه‌ی این‌جا
    # پاک‌سازی‌شده (فقط سرستون + داده) جایگزین شد، نه فایل خام کاربر.
    {"file": "Kazakhstan_Companies.xlsx", "country": "قزاقستان"},
]
...
FIELD_KEYWORDS = {
    # «نام انگلیسی» اضافه شد چون فایل قزاقستان (۲۰۲۶-۰۹-۱۶) این عبارت رو داشت،
    # نه «نام شرکت»/«نام کمپانی» — بدون این کلیدواژه، ستون english_name اصلا
    # پیدا نمی‌شد و load_companies (که ردیف بدون english_name رو رد می‌کنه)
    # کل ۴۱ شرکت رو ساکت حذف می‌کرد.
    "english_name": ["english name", "نام شرکت", "نام کمپانی", "نام انگلیسی"],
    ...
    "action_plan": ["action plan", "برنامه اقدام", "جزئیات فعالیت", "زمینه دقیق مصرف"],
