...
SEED_NEWS_SOURCES = [
    ...
    # --- فهرست دوم منابع روسیه (کاربر، صحت‌سنجی‌شده ۲۰۲۶-۰۹-۰۷) ---
    # همه با fetch_page_text واقعی همین فایل تست شدند (نه فقط ابزار WebFetch
    # جدا که مکانیزم متفاوتی داره). rccnews قبلاً با WebFetch بی‌نتیجه به‌نظر
    # می‌رسید ولی با urllib خودمون کار می‌کنه — پس تست هر منبع باید حتماً با
    # همین fetch_page_text باشد.
    "https://rccnews.ru/",  # RCCNews - اخبار روزانه‌ی بازار شیمیایی روسیه/CIS، بخش پرایس‌لیست و آگروشیمی (تأیید ۲۰۲۶-۰۹-۰۷)
    "https://www.soda.ru/ru/",  # БСК (Bashkir Soda Company) - بزرگ‌ترین تولیدکننده‌ی جوش شیرین روسیه، زیرمجموعه‌ی هلدینگ روسخیم (Roskhim) (تأیید ۲۰۲۶-۰۹-۰۷)
    "https://www.rzd-partner.ru/",  # РЖД-Партнер - تعرفه‌های ریلی روسیه، بخش اختصاصی صنعت شیمی (تأیید ۲۰۲۶-۰۹-۰۷)
    "https://trans.ru/",  # Trans.ru - بورس‌بار جاده‌ای، بخشنامه‌های حمل‌ونقل، عوارض جاده‌ای/ترانزیت (تأیید ۲۰۲۶-۰۹-۰۷)
    "https://flagma.ru/",  # Flagma - آگهی‌های خرید/فروش عمده؛ رقبای ایرانی هم این‌جا فعالند (تأیید ۲۰۲۶-۰۹-۰۷ با urllib خام؛ در مرورگر واقعی محافظت ضدربات داره ولی fetch ساده رد می‌شه)
    "http://board.himtrade.ru/",  # HimTrade - تابلوی اعلانات B2B شیمیایی از ۲۰۰۳ (تأیید ۲۰۲۶-۰۹-۰۷؛ صفحه Windows-1251 است، فقط بعد از رفع تشخیص انکودینگ در fetch_utils.py قابل‌خوندن شد)
    # --- منابع اخبار چین (اضافه‌شده ۲۰۲۶-۰۹-۱۳) ---
    # همه با fetch_page_text واقعی همین فایل تست شدند. قبل از این‌ها، sunsirs.com
    # و 100ppi.com (دو شاخص قیمت داخلی معروف چین) و echemi.com/informations/
    # امتحان و رد شدند: دوتای اول پشت یک چالش امنیتی جاوااسکریپتی هوآوی‌کلود
    # قفل‌اند (شبیه Argus/ICIS که قبلاً رد شده بودن)، سومی محتواش سمت کلاینت
    # رمزنگاری شده و با fetch ساده غیرقابل‌استخراجه (صفحه‌ی قیمت echemi که برای
    # price_intelligence_bot.py استفاده می‌کنیم مشکلی نداره، فقط بخش اخبارش نه).
    "https://www.caixinglobal.com/",  # Caixin Global - اقتصاد/تجارت چین به انگلیسی، شامل جنگ تجاری آمریکا-چین (تأیید ۲۰۲۶-۰۹-۱۳؛ خلاصه‌ی مقاله‌ها آزاده، متن کامل احتمالاً پی‌وال)
    "https://www.scmp.com/business",  # South China Morning Post - اقتصاد/تجارت چین و هنگ‌کنگ (تأیید ۲۰۲۶-۰۹-۱۳)
    ...
    # --- منبع اقتصادی رسمی ایران، چندکشوره (کاربر، ۲۰۲۶-۰۹-۱۶) ---
    # کاربر ابتدا فقط صفحه‌ی اختصاصی قزاقستان این سایت رو داد؛ بعد مشخص شد
    # منظورش کل سایته («اخبار اقتصادی جاهای مختلف رو بررسی می‌کنه»). سایت
    # ده‌ها زیرصفحه‌ی تک‌کشوری داره (newsagencyshow/<id>)، ولی همون IDها فقط
    # از منوی مگامنو (که با AJAX لود می‌شه، توی HTML خام نیست) قابل‌کشفن —
    # پس به‌جای حدس‌زدن ده‌ها ID، خودِ صفحه‌ی اصلی اضافه شد: یک فید چرخشی از
    # جدیدترین اخبار روی همه‌ی مناطق/کشورها با هم (تست ۲۰۲۶-۰۹-۱۶ واقعاً
    # ترکمنستان+ایران+چین+عربستان+ساحل‌عاج+پاکستان+اندونزی+کامرون رو هم‌زمان
    # نشون داد) — دقیقاً همون چیزی که خواسته شده بود، بدون نیاز به فهرست
    # دستی چندین ID جدا.
    #
    ...
    # سایت خیلی طولانیه — قبل از رسیدن به اخبار واقعی حدود ۶۳۰۰ کاراکتر منو
    # میاد، پس max_chars این فراخوانِ خاص (نه پیش‌فرض سراسری fetch_utils) به
    # ۱۲٬۰۰۰ افزایش یافت، وگرنه هیچ خبر واقعی‌ای به مدل نمی‌رسید.
    "https://economic.mfa.ir/",  # معاونت دیپلماسی اقتصادی وزارت امور خارجه - فید چندکشوره‌ی صفحه‌ی اصلی (تأیید ۲۰۲۶-۰۹-۱۶)
]
...
# بررسی شد و عمداً اضافه نشد (چین، ۲۰۲۶-۰۹-۱۳): انجمن نمک‌های معدنی چین
# (cnisa.org.cn، دقیقاً مرتبط با جوش شیرین/سودا اش) - روی http با 503 و روی
# https با خطای SSL (UNEXPECTED_EOF_WHILE_READING) جواب داد؛ این‌بار مشکل
# ضدربات نیست، به‌نظر خودِ سرور/گواهی سایت خرابه. اگه بعداً دوباره تست کردید
# و کار کرد (ممکنه موقتی بوده)، برگردونید. cnchemicals.com (پلتفرم دیتای CCM)
# هم رد شد چون صفحه‌ی اصلیش محتوای خبری واقعی ندارد، بیشتر تبلیغ دیتابیس/
# فروش گزارشه. chemweek.com هم رد شد چون کاملاً با جاوااسکریپت رندر می‌شه
# (متن خام تقریباً خالیه، غیرقابل‌fetch با urllib).
...
(دو تولیدکننده‌ی مصنوعی سولوی-آمونیاکی — BSK و Krymsoda؛ صادرات جهانی‌اش از ۲۰۲۱
حدود ۶۴٪ سقوط کرده، ولی به‌خاطر تعرفه‌ی صفر اتحادیه‌ی اقتصادی اوراسیا (EAEU) و
ریلی مشترک شوروی، در بازارهای CIS/آسیای میانه/قفقاز — به‌خصوص ارمنستان و قزاقستان —
رقیب مهم و نزدیکی برای ماست).
...
- حداکثر ۱۲۰ کلمه.
- زمینه‌ی شرکت (قیمت پایه‌ی ۲۵۰ دلار، رقبا) رو مثل یک تحلیلگر واقعی توی ذهنت
  نگه دار تا تحلیلت دقیق‌تر و مرتبط‌تر باشه — نه اینکه هر بار عیناً بنویسیش.
  **فقط وقتی خبر واقعاً به قیمت/رقابت ما ربط مستقیم داره** یک جمله درباره‌ی
  تاثیرش بیار؛ اگر ربطی نداره (مثلاً خبر صرفاً رویدادیه)، این جمله رو اضافه
  نکن. هرگز از یک عبارت کلیشه‌ای ثابت (مثل «با توجه به قیمت پایه‌ی ۲۵۰ دلاری
  FOB») در چند آیتم پشت‌سرهم استفاده نکن — هر بار با زبان و زاویه‌ی متفاوت
  بنویس، دقیقاً همون‌طور که یک تحلیلگر انسانی هیچ‌وقت یک جمله رو کلمه‌به‌کلمه
