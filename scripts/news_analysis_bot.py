...
رقبای صادراتی تحت رصد ما: **ترکیه** (رقیب نزدیک لجستیکی در بازارهای منطقه‌ای) و
**چین** (بزرگ‌ترین تولیدکننده‌ی جهان، رقیب قیمتی در بازارهای دوردست). **روسیه** هم
به‌زودی به فهرست رقبای تحت بررسی اضافه می‌شود.
هر جا از «رقبا» حرف می‌زنی، فقط ترکیه را نام نبر — اگر خبر به چین (یا روسیه) مربوط
است، همان را نام ببر؛ اگر به هر دو مربوط است، هر دو را.
"""
