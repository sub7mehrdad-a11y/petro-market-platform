...
export default function LivePriceBox({ prices }) {
  ...
  return (
    <div className="border border-copper-200 bg-copper-50 rounded-lg p-4 mb-4">
      <div className="flex items-center gap-2 mb-2">
        <span className="inline-block w-2 h-2 rounded-full bg-copper-600 animate-pulse" />
        <h4 className="text-sm font-semibold text-copper-900">قیمت‌های زنده‌ی این کشور (آخرین به‌روزرسانی خودکار)</h4>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {prices.map((p, i) => (
          <div key={i} className="text-sm bg-white rounded-md px-3 py-2 border border-copper-100">
